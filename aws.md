aws ec2 우분투 설정과 aws 과금 파트 각각 다른 md으로 나눌 예정입니다 과금파트는 지금 제 담배값을 날리면서까지 분석중이니 기다려주십쇼
---
...
| 항목 | 설명 | 비고 |
|------|------|------|
| **인스턴스 타입** | `t2.micro`만 프리티어 대상<br> → `t2.medium`, `t3.small` 등은 **프리티어 적용 ❌** | 난 현재 `t2.medium` 사용 중 → 유료 24시간 돌리면 2천원정도|
...
| 시나리오                                | 트래픽 발생?  | 설명                      |
| ----------------------------------- | -------- | ----------------------- |
| 유저가 내 사이트 들어옴                       | ✅ 발생     | HTML, JS, CSS 전송        |
| 유저가 이미지 봄 (`/images/board/xxx.jpg`) | ✅ 발생     | 이미지 파일 전송 (용량큼 과금됨)     |
| 유저가 게시판에서 글 여러 개 봄                  | ✅ 발생     | 계속 HTML, 이미지 등 나감       |
| 유저가 뭔가 올림 (예: 이미지 업로드)              | ❌ 과금 안 됨 | **들어오는 데이터는 무료 (인바운드)** |
| 내가 EC2에서 DB 조회함                     | ❌        | 내부통신이라 과금 없음            |

- 프리티어여도 트래픽 1gb넘으면 과금발생
> - 이 기준은 월별 트래픽 1gb 기준
> - 사실상 과금 확정임 물론 비싼수준은 아님
> - 즉 우리 프로젝트는 과금이 프리티어든 뭐던 무조건 발생하는 프로젝트다 그렇다면 그냥 돈써서 인스턴스 업그레이드 후 테스트를 하자
>     - 미련하게 무료버전으로 하면 정신건강에 안좋음 프로젝트 규모상 어쩔 수 없는 판단
