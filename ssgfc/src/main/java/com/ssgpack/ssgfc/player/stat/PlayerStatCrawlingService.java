...
public class PlayerStatCrawlingService {
    ...
    public void crawlAndSavePlayerStat(String url, Player player) throws IOException {
        ...
        String position = savedPlayer.getPosition();

        Elements rows = table.select("tbody > tr");
        for (Element row : rows) {
            Elements tds = row.select("td");
            String season = tds.get(1).text();

            // 🧢 타자 처리
            if (position != null && !position.contains("투수")) {
                if (tds.size() >= 34) {
                    ...
                    if (existing.isPresent()) {
                        System.out.println("✔ 이미 저장된 타자 기록: " + savedPlayer.getName() + " (" + season + ")");
                        continue;
                    }
                    ...
                    System.out.println("📊 타자 기록 저장 완료: " + savedPlayer.getName() + " (" + season + ")");
                }

            } else {
                // ⚾ 투수 처리
                if (tds.size() >= 27) {
                    Double era = parseDouble(tds.get(18).text());
                    Double whip = parseDouble(tds.get(19).text());
                    Double war = parseDouble(tds.get(26).text());

                    Optional<PlayerStat> existing = playerStatRepository.findByPlayerAndSeason(savedPlayer, season);
                    if (existing.isPresent()) {
                        System.out.println("✔ 이미 저장된 투수 기록: " + savedPlayer.getName() + " (" + season + ")");
                        continue;
                    }
                    ...
                    playerStatRepository.save(stat);
                    System.out.println("📊 투수 기록 저장 완료: " + savedPlayer.getName() + " (" + season + ")");
                }
            }
        }
    }
